...
## 2026-05-19 — E2E navigation helpers duplicated across asset test files

- Found by: reviewer-e2e
- Where: e2e/assets/asset_exchange.test.ts, e2e/assets/assets.test.ts, e2e/asset_web_lookup/asset_web_lookup.test.ts
- Context: branch `feat/asset-exchange-e2e` @ `5ad927a`
- Severity: 🟡
- Observation: `navigateToAssets()` is inlined identically in three E2E spec files (asset_exchange, assets, asset_web_lookup). Same FAB selector strategy and timeout. Any change to the nav button selector or the wait shape has to be applied in three places independently.
